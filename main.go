...
func getCustomers(w http.ResponseWriter, r *http.Request) {
	...
	for i := 0; i < len(request.Orders.Order); i++ {
		customerID2 := request.Orders.Order[i].CustomerID
		employeeID2 := request.Orders.Order[i].EmployeeID
		...
		_, err = stmt.Exec(customerID2, employeeID2, orderDate, requiredDate, shippedDate, shipVia, freight, shipName, shipAddress, shipCity, shipRegion, shipPostalCode, shipCountry)

		if err != nil {
			fmt.Fprintln(w, "Data", customerID2, " and Employee ID: ", employeeID2, " Duplicate")
		} else {
			fmt.Fprintln(w, "Data Customer ID:", customerID2, "and Employee ID:", employeeID2, " Created")
		}
	}
}
